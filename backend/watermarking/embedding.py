...
def resize2(image_path):
    ...
    pad_w = M - W
    padded = np.pad(img, ((0, pad_h), (0, pad_w)), mode='constant', constant_values=0)
    return padded.astype(np.float32), (H, W, pad_h, pad_w), M
...
def _fitness(alpha_mat, HSw_list, wm_sv_list, *args):
    lam = 0.6
    HSw = np.array(HSw_list)
    SW  = np.array(wm_sv_list)
...
def embed_watermark(
    ...
    pso_iters: int = 50,
    # FIX BUG 3: Widened alpha bounds so division by alpha doesn't amplify noise 1000×
    alpha_bounds: tuple = (0.01, 0.1),
    ...
    try:
        ...
        I, pad_info, h1 = resize2(host_path)
        ...
        # =====================================================
        # 🔹 SAVE KEY (.npz)
        # FIX BUG 2: HSw_list saved as float64 (was dtype=object → caused
        #            silent shape/dtype mismatches on np.load and broken
        #            subtraction during extraction)
        # FIX BUG 4: Sw_full now saved in key so extraction can reconstruct
        #            with the correct full-length SV vector instead of
        #            zero-padding from sv_len=8 up to wm_side
        # =====================================================
        ...
        np.savez(
            ...
            Uw=Uw,
            Sw_full=Sw_full,          # FIX BUG 4: was missing — needed for full ISVD reconstruction
            Vtw=Vtw,
            ...
            HSw_list=np.array(HSw_list, dtype=np.float64),  # FIX BUG 2: was dtype=object
            orig_H=orig_H,
            orig_W=orig_W,
            pad_h=pad_h,
            pad_w=pad_w,
...
#         # save_image(path_original_watermarked(process),Iw_uint8)
#         save_image(path_output(process), Iw_cropped)
